...
public class Handler {
     ...
     *
     * 初始化Handler对象 & 绑定线程
     * 1.Handler需绑定线程才能使用，绑定后Handler的消息处理会在绑定的线程中执行
     * 2.绑定方式 = 先指定Looper对象，从而绑定了Looper对象所绑定的线程(因为Looper对象本已绑定了对应线程)
     * 3.即：注定了Handler对象的Looper对象 = 绑定到了Looper对象所在的线程
     */
    public Handler() {
        this(null, false);
    }
    ...
    public Handler(Callback callback, boolean async) {
		...
		// 至此，保证了Handler对象关联上Looper对象中的MessageQueue
     ...
     *
     * 把Runnable r添加进消息队列
     * Runnable对象会在handler依附的线程上执行
     *
     * @参数 r 要执行的Runnable
     *
     * @返回 如果Runnable成功添加进消息队列则返回true
     *       如果失败则返回false，通常是因为Looper处理的消息队列正在推出
     */
	...
	/**
	 * 把Runnable参数封装进Message的callback属性
	 */
